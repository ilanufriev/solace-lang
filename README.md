...
1. **Язык и компилятор (`compiler`)**
   - читает исходный `.solace`, парсит грамматикой ANTLR и проверяет корректность сети (узлы, порты, соединения);
   - генерирует байткод для hardware/software‑узлов и упаковывает его вместе с описанием сети в единый бинарный пакет `*.solpkg` (формат см. `FILE_FORMATS.md`).

2. **Узловые виртуальные машины (`vm/sim`, `vm/harv`)**
   - исполняют байткод конкретных узлов, реализуя семантику hardware‑ и software‑узлов;
   - не знают о структуре всей сети, работают только с собственными портами и FIFO‑каналами.

3. **Сетевая ВМ (`vm/network`)**
   - читает пакет `*.solpkg`, восстанавливает список узлов и соединений между ними;
   - создаёт каналы сети Кана и запускает для каждого узла отдельную ВМ (stub/sim/harv или смешанную) в своей корутине.

Жизненный цикл программы:

1. Компилятор: `program.solace → program.solpkg` (один файл с метаданными сети и байткодом всех узлов).
2. Сетевая ВМ: загружает `program.solpkg`, строит сеть, создаёт ВМ узлов и для каждого узла выполняет `init` один раз, затем бесконечный цикл `run`, обмениваясь данными только через FIFO‑каналы.
...
- [Компилятор (`compiler`)](compiler/README.md) — разбор `.solace` и генерация пакета `*.solpkg`.
- [Сетевая ВМ (`vm/network`)](vm/network/README.md) — загрузка пакета, построение сети Кана и запуск узловых ВМ.
- [Симулятор hardware‑узлов (`vm/sim`)](vm/sim/README.md) — исполнение байткода аппаратных узлов.
- [Стековая ВМ Harv (`vm/harv`)](vm/harv/README.MD) — стековая машина для software‑узлов и программных сетей.
