...
- синтаксический и семантический анализ программы на Solace (узлы, порты, связи, аппаратный и программный код);
- генерация бинарного пакета `SOLP` (см. `FILE_FORMATS.md`), содержащего:
  - метаданные о сети (узлы, типы, порты, соединения);
  - solbc‑контейнеры (`SOLB`) с байткодом для аппаратных (`NodeType.HARDWARE`) и программных (`NodeType.SOFTWARE`) узлов.
...
- Генерация байткода для программных узлов:
  - обход AST программных узлов `SoftwareVisitor`;
  - построение списка инструкций стековой машины для секций `init` и `run`;
  - кодирование инструкций в строковое представление (через `AsmParser.encodeInstructions`);
  - упаковка инструкций в solbc‑контейнеры (`SOLB`) с пометкой типа `SOFTWARE`.
...
- Удобство диагностики:
  ...
  - разделение ошибок синтаксиса (ANTLR), семантики (`ValidationException`) и ошибок бэкендов (`HardwareVisitorException`, `SoftwareVisitorException`).
...
Запуск из Gradle (из корня репозитория):

- пример: `./gradlew :compiler:run --args "examples/fibonacci_mixed.solace --out build/solace"`
- без аргументов: `./gradlew :compiler:run` (будет использован `../pseudocode.solace`);
- для печати AST: `./gradlew :compiler:run --args "examples/fibonacci_mixed.solace --print-ast"`.
...
- `App.kt`  
  ...
  - вызывает `compileFile`, которая:
    ...
    - при отсутствии синтаксических ошибок:
      - вызывает `buildHardwareBytecode` для генерации байткода аппаратных узлов;
      - вызывает `buildSoftwareBytecode` для генерации байткода программных узлов;
      - объединяет результаты в единую карту `bytecodeByNode`;
...
- `softwareCompilation.kt`  
  Слой компиляции программных узлов:
  - обходит AST с помощью `SoftwareVisitor`, получая список описаний software‑узлов;
  - для каждого программного узла:
    - кодирует init/run‑инструкции через `AsmParser.encodeInstructions`;
    - заворачивает их в `buildSolbcContainer(NodeType.SOFTWARE, init, run)`.
...
- `SoftwareVisitor` — анализирует программные узлы (`software`), собирает их сигнатуру (порты, self‑FIFO, локальные переменные) и генерирует список инструкций для стековой ВМ.

Взаимодействие модулей:

- `App.main` → `parseTopology` → `analyzeProgram` → `buildHardwareBytecode` + `buildSoftwareBytecode` → `writeProgramPackage`.
- Сгенерированный `*.solpkg` затем используется `vm/network`:
  - `loadProgramPackage` → `buildNetwork` → запуск аппаратных и программных узлов.
...
- `compileFile`:
  ...
  - в блоке `try`:
    ...
    - `buildSoftwareBytecode(tree)` — генерация байткода для программных узлов;
    - объединение карт байткода аппаратных и программных узлов в единый `bytecodeByNode`;
  ...
  - ловит и диагностирует:
    ...
    - `SoftwareVisitorException` — ошибки программной части.
- `resolveInputPath`:
...
### Генерация программного байткода (`softwareCompilation.kt`)

Ключевые элементы:

- `buildSoftwareBytecode(program: ProgramContext)`:
  - создаёт `SoftwareVisitor` и прогоняет его по всему AST;
  - результат приводится к списку `SoftwareVisitor.Node`;
  - делегирует работу перегрузке `buildSoftwareBytecode(nodes)`.
- `buildSoftwareBytecode(nodes: List<SoftwareVisitor.Node>)`:
  - для каждого программного узла:
    - извлекает имя, init‑ и run‑код в терминах инструкций стековой ВМ;
    - проверяет уникальность имён узлов (запрещает дубликаты);
    - кодирует инструкции через `AsmParser.encodeInstructions` и сворачивает в строки;
    - вызывает `buildSolbcContainer(NodeType.SOFTWARE, initBytes, runBytes)`;
  - возвращает карту `nodeName -> solbc‑байткод`.
...
- `SoftwareVisitorTest`:
  - проверяет, что `SoftwareVisitor`:
    - корректно разбирает программные узлы (порты, self‑очереди, локальные переменные);
    - генерирует байткод для стековой ВМ;
    - загруженный байткод корректно исполняется `StackMachine` (проверяются статусы исполнения и содержимое FIFO).
...
- корректность генерации аппаратного и программного байткода и его исполнения симулятором/стековой ВМ;
